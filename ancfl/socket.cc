...
Socket::ptr Socket::CreateTCP(ancfl::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(ancfl::Address::ptr address) {
...
Socket::ptr Socket::accept() {
    ...
    if (newsock == -1) {
        ANCFL_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (!isValid()) {
        newSock();
        if (ANCFL_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if (ANCFL_UNLIKELY(addr->getFamily() != m_family)) {
    ...
    if (uaddr) {
        ...
        if (sock->connect(uaddr)) {
            ...
            ancfl::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if (!m_remoteAddress) {
        ANCFL_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::listen(int backlog) {
    if (!isValid()) {
        ANCFL_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, ancfl::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, ancfl::IOManager::WRITE);
}
