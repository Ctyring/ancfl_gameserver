...
bool Index::init(const tinyxml2::XMLElement& node) {
    if (!node.Attribute("name")) {
        ANCFL_LOG_ERROR(g_logger) << "index name not exists";
        return false;
    }
    m_name = node.Attribute("name");

    if (!node.Attribute("type")) {
        ANCFL_LOG_ERROR(g_logger) << "index name=" << m_name << " type is null";
        return false;
    }
    ...
    if (!node.Attribute("cols")) {
        ANCFL_LOG_ERROR(g_logger) << "index name=" << m_name << " cols is null";
    }
    std::string tmp = node.Attribute("cols");
    m_cols = ancfl::split(tmp, ',');
