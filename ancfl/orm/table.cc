...
static ancfl::Logger::ptr g_logger = ANCFL_LOG_NAME("orm");

std::string Table::getFilename() const {
    return ancfl::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if (!node.Attribute("name")) {
        ANCFL_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if (!node.Attribute("namespace")) {
        ANCFL_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + ancfl::replace(m_namespace, ".", "/");
    ancfl::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}
...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"ancfl/db/db.h", "ancfl/util.h"};
    for (auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = ancfl::split(m_namespace, '.');
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for (auto it = m_cols.begin(); it != m_cols.end(); ++it) {
        ...
        if ((*it)->getDType() == Column::TYPE_UINT64 ||
            ...
            ss << "ancfl::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
               << ";" << std::endl;
        } else {
            ss << GetAsMemberName((*it)->getName()) << ";" << std::endl;
        }
    }
    ss << "    return ancfl::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"ancfl/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static ancfl::Logger::ptr g_logger = ANCFL_LOG_NAME(\"orm\");"
        << std::endl;
...
#define CHECK_STMT(v)                                                   \
    ...
    ofs << "        ANCFL_LOG_ERROR(g_logger) << \"stmt=\" << sql"      \
        << std::endl                                                    \
        << "                 << \" errno=\""                            \
