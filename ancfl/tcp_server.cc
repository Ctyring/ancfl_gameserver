...
static ancfl::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    ancfl::Config::Lookup("tcp_server.read_timeout",
                          (uint64_t)(60 * 1000 * 2),
                          "tcp server read timeout");

static ancfl::Logger::ptr g_logger = ANCFL_LOG_NAME("system");

TcpServer::TcpServer(ancfl::IOManager* worker,
...
bool TcpServer::bind(ancfl::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
    ...
    for (auto& addr : addrs) {
        ...
        // 监听
        if (!sock->listen()) {
            ANCFL_LOG_ERROR(g_logger) << "listen fail errno=" << errno
