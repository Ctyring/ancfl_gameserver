...
bool NameServerModule::handleRockRequest(ancfl::RockRequest::ptr request,
                                         ancfl::RockResponse::ptr response,
                                         ancfl::RockStream::ptr stream) {
    ancfl::Atomic::addFetch(s_request_count, 1);
...
bool NameServerModule::handleRockNotify(ancfl::RockNotify::ptr notify,
                                        ancfl::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(ancfl::Stream::ptr stream) {
    ancfl::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if (!rockstream) {
        ANCFL_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        ANCFL_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(ancfl::Stream::ptr stream) {
    ancfl::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if (!rockstream) {
        ANCFL_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        ANCFL_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(ancfl::RockStream::ptr rs) {
    ancfl::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(ancfl::RockRequest::ptr request,
                                      ancfl::RockResponse::ptr response,
                                      ancfl::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if (!rr) {
        ANCFL_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(ancfl::RockStream::ptr rs,
                           NSClientInfo::ptr new_value) {
    if (!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    // ancfl::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<ancfl::RockStream::ptr> NameServerModule::getStreams(
    const std::string& domain) {
    ancfl::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<ancfl::RockStream::ptr>()
                                          : it->second;
}
...
bool NameServerModule::handleQuery(ancfl::RockRequest::ptr request,
                                   ancfl::RockResponse::ptr response,
                                   ancfl::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if (!qreq) {
        ANCFL_LOG_ERROR(g_logger) << "invalid query request from: "
                                  << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(ancfl::RockStream::ptr rs,
...
bool NameServerModule::handleTick(ancfl::RockRequest::ptr request,
                                  ancfl::RockResponse::ptr response,
                                  ancfl::RockStream::ptr stream) {
    return true;
}
