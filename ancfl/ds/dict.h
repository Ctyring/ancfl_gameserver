#ifndef __ANCFL_DS_DICT_H__
#define __ANCFL_DS_DICT_H__

#include <functional>
#include <iostream>
#include <memory>
#include "ancfl/ds/util.h"
#include "ancfl/log.h"
...
class StringDict;
template <class K, class V, class PosHash = ancfl::ds::Murmur3Hash<K> >
class Dict {
   ...
   public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        ancfl::RWMutex::ReadLock lock2(s_mutex[pos % MAX_MUTEX]);
        auto it =
            BinarySearch(m_datas[pos].begin(), m_datas[pos].end(), Node(k));
        // ANCFL_ASSERT(it == std::find(m_datas[pos].begin(),
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        ancfl::RWMutex::ReadLock lock(m_mutex);
        uint32_t pos = hashvalue % m_size;
        ancfl::RWMutex::WriteLock lock2(s_mutex[pos % MAX_MUTEX]);
    ...
    void foreach (callback cb) {
        ...
        for (size_t i = 0; i < m_size; ++i) {
            ancfl::RWMutex::ReadLock lock2(s_mutex[i % MAX_MUTEX]);
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if (speed == (uint64_t)-1) {
            ...
            ancfl::WriteFixToStreamWithSpeed(os, (const char*)&vs[0], size,
                                             speed);
        }
        ...
        if (speed == (uint64_t)-1) {
            ...
            ancfl::WriteFixToStreamWithSpeed(os, (const char*)&ns[0], size,
                                             speed);
        }
   ...
   private:
    ...
    static ancfl::RWMutex s_mutex[MAX_MUTEX];
};

template <class K, class V, class PosHash>
ancfl::RWMutex Dict<K, V, PosHash>::s_mutex[MAX_MUTEX];
...
class StringDict {
   public:
    static uint64_t GetID(const std::string& str) {
        ...
        return ancfl::murmur3_hash64(str.c_str(), str.size(), 1060627423,
                                     1050126127);
    }
    static uint64_t GetID(const char* str, const uint32_t& size) {
        ...
        return ancfl::murmur3_hash64(str, size, 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str) {
        if (str == nullptr) {
            return 0;
        }
        return ancfl::murmur3_hash64(str, 1060627423, 1050126127);
    }
