...
Module::ptr Library::GetModule(const std::string& path) {
    ...
    if (!create) {
        ANCFL_LOG_ERROR(g_logger) << "cannot load symbol CreateModule in "
                                  << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    if (!destory) {
        ANCFL_LOG_ERROR(g_logger) << "cannot load symbol DestoryModule in "
                                  << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    ANCFL_LOG_INFO(g_logger) << "load module name=" << module->getName()
                             << " version=" << module->getVersion()
                             << " path=" << module->getFilename() << " success";
    Config::LoadFromConfDir(ancfl::EnvMgr::GetInstance()->getConfigPath(),
                            true);
    return module;
}
