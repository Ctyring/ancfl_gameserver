...
static ancfl::ConfigVar<uint32_t>::ptr g_rock_protocol_max_length =
    ancfl::Config::Lookup("rock.protocol.max_length",
                          (uint32_t)(1024 * 1024 * 64),
                          "rock protocol max length");

static ancfl::ConfigVar<uint32_t>::ptr g_rock_protocol_gzip_min_length =
    ancfl::Config::Lookup("rock.protocol.gzip_min_length",
                          (uint32_t)(1024 * 4),
...
bool RockRequest::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockRequest serializeToByteArray error";
    }
    return false;
}
...
bool RockRequest::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockRequest parseFromByteArray error";
    }
    return false;
}
...
bool RockResponse::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockResponse serializeToByteArray error";
    }
    return false;
}
...
bool RockResponse::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockResponse parseFromByteArray error";
    }
    return false;
}
...
bool RockNotify::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockNotify serializeToByteArray error";
    }
    return false;
}
...
bool RockNotify::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockNotify parseFromByteArray error";
    }
    return false;
}
...
Message::ptr RockMessageDecoder::parseFrom(Stream::ptr stream) {
    try {
        ...
        if (stream->readFixSize(&header, sizeof(header)) <= 0) {
            ANCFL_LOG_ERROR(g_logger) << "RockMessageDecoder decode head error";
            return nullptr;
        }

        // 校验头部魔数
        if (memcmp(header.magic, s_rock_magic, sizeof(s_rock_magic))) {
            ANCFL_LOG_ERROR(g_logger) << "RockMessageDecoder head.magic error";
            return nullptr;
        }
        ...
        // 校验长度
        header.length = ancfl::byteswapOnLittleEndian(header.length);
        ...
        if (header.flag & 0x1) {  // gizp
            auto zstream = ancfl::ZlibStream::CreateGzip(false);
            // 写入数据
            if (zstream->write(ba, -1) != Z_OK) {
                ANCFL_LOG_ERROR(g_logger) << "RockMessageDecoder ungzip error";
                return nullptr;
            }
        ...
        ANCFL_LOG_ERROR(g_logger) << "RockMessageDecoder except:" << e.what();
    } catch (...) {
        ANCFL_LOG_ERROR(g_logger) << "RockMessageDecoder except";
    }
    return nullptr;
}
...
int32_t RockMessageDecoder::serializeTo(Stream::ptr stream, Message::ptr msg) {
    ...
    if ((uint32_t)header.length >=
        ...
        auto zstream = ancfl::ZlibStream::CreateGzip(true);
        if (zstream->write(ba, -1) != Z_OK) {
            ANCFL_LOG_ERROR(g_logger)
