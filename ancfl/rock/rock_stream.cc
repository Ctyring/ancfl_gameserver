...
static ancfl::ConfigVar<
    ...
    g_rock_services = ancfl::Config::Lookup(
        "rock_services",
        std::unordered_map<std::string,
                           std::unordered_map<std::string, std::string> >(),
        "rock_services");

// static ancfl::ConfigVar<std::unordered_map<std::string
//     ,std::unordered_map<std::string, std::string> > >::ptr g_rock_services =
//     ancfl::Config::Lookup("rock_services", std::unordered_map<std::string
...
RockStream::RockStream(Socket::ptr sock)
    ...
    ANCFL_LOG_DEBUG(g_logger) << "RockStream::RockStream " << this << " "
                              << (sock ? sock->toString() : "");
}
...
RockResult::ptr RockStream::request(RockRequest::ptr req, uint32_t timeout_ms) {
    if (isConnected()) {
        ...
        addCtx(ctx);
        uint64_t ts = ancfl::GetCurrentMS();
        ctx->timer = ancfl::IOManager::GetThis()->addTimer(
            timeout_ms,
            std::bind(&RockStream::onTimeOut, shared_from_this(), ctx));
        enqueue(ctx);
        ancfl::Fiber::YieldToHold();
        return std::make_shared<RockResult>(
            ctx->result, ancfl::GetCurrentMS() - ts, ctx->response, req);
    } else {
...
AsyncSocketStream::Ctx::ptr RockStream::doRecv() {
    // ANCFL_LOG_INFO(g_logger) << "doRecv " << this;
    ...
    if (type == Message::RESPONSE) {
        ...
        if (m_requestHandler) {
            ...
            ANCFL_LOG_WARN(g_logger) << "unhandle request " << req->toString();
        }
        ...
        if (m_notifyHandler) {
            ...
            ANCFL_LOG_WARN(g_logger) << "unhandle notify " << nty->toString();
        }
    } else {
        ANCFL_LOG_WARN(g_logger) << "RockStream recv unknow type=" << type
                                 << " msg: " << msg->toString();
    }
    return nullptr;
}

void RockStream::handleRequest(ancfl::RockRequest::ptr req) {
    ancfl::RockResponse::ptr rsp = req->createResponse();
    if (!m_requestHandler(
            req, rsp,
            std::dynamic_pointer_cast<RockStream>(shared_from_this()))) {
        sendMessage(rsp);
...
void RockStream::handleNotify(ancfl::RockNotify::ptr nty) {
    if (!m_notifyHandler(
...
bool RockConnection::connect(ancfl::Address::ptr addr) {
    m_socket = ancfl::Socket::CreateTCP(addr);
...
static SocketStream::ptr create_rock_stream(ServiceItemInfo::ptr info) {
    ancfl::IPAddress::ptr addr =
        ancfl::Address::LookupAnyIPAddress(info->getIp());
    if (!addr) {
    ...
    ancfl::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr]() {
        conn->connect(addr);
        conn->start();
    });
    return conn;
...
RockResult::ptr RockSDLoadBalance::request(const std::string& domain,
    ...
    uint64_t ts2 = ancfl::GetCurrentMS();
    if (r->result == 0) {
        stats.incOks(1);
