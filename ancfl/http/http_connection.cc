...
HttpConnection::~HttpConnection() {
    ANCFL_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if (!body.empty()) {
        ...
        ANCFL_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
                                  << " size=" << body.size();
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri,
    ...
    if (!turi) {
        ANCFL_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = ancfl::GetCurrentMS();
    ...
    if (!ptr) {
        ...
        if (!addr) {
            ANCFL_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock =
        ...
        if (!sock) {
            ANCFL_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if (!sock->connect(addr)) {
            ANCFL_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr,
    ...
    ++ptr->m_request;
    if (!ptr->isConnected() ||
        ((ptr->m_createTime + pool->m_maxAliveTime) >= ancfl::GetCurrentMS()) ||
