...
ancfl::ConfigVar<uint32_t>::ptr g_websocket_message_max_size =
    ancfl::Config::Lookup("websocket.message.max_size",
...
HttpRequest::ptr WSSession::handleShake() {
    ...
    do {
        req = recvRequest();
        if (!req) {
            ANCFL_LOG_INFO(g_logger) << "invalid http request";
            break;
        }
        if (strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
            ANCFL_LOG_INFO(g_logger) << "http header Upgrade != websocket";
            break;
        }
        if (strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
            ANCFL_LOG_INFO(g_logger) << "http header Connection != Upgrade";
            break;
        }
        ...
        if (key.empty()) {
            ANCFL_LOG_INFO(g_logger) << "http header Sec-WebSocket-Key = null";
            break;
        }

        std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
        v = ancfl::base64encode(ancfl::sha1sum(v));
        ...
        sendResponse(rsp);
        ANCFL_LOG_DEBUG(g_logger) << *req;
        ANCFL_LOG_DEBUG(g_logger) << *rsp;
        return req;
    } while (false);
    if (req) {
        ANCFL_LOG_INFO(g_logger) << *req;
    }
    return nullptr;
}
...
WSFrameMessage::ptr WSRecvMessage(Stream* stream, bool client) {
    ...
    do {
        ...
        ANCFL_LOG_DEBUG(g_logger) << "WSFrameHead " << ws_head.toString();

        if (ws_head.opcode == WSFrameHead::PING) {
            ANCFL_LOG_INFO(g_logger) << "PING";
            if (WSPong(stream) <= 0) {
                break;
            }
            ...
            if (!client && !ws_head.mask) {
                ANCFL_LOG_INFO(g_logger) << "WSFrameHead mask != 1";
                break;
            }
            uint64_t length = 0;
            if (ws_head.payload == 126) {
                ...
                length = ancfl::byteswapOnLittleEndian(len);
            ...
            if (ws_head.fin) {
                ANCFL_LOG_DEBUG(g_logger) << data;
                return WSFrameMessage::ptr(
                    new WSFrameMessage(opcode, std::move(data)));
            }
        } else {
            ANCFL_LOG_DEBUG(g_logger) << "invalid opcode=" << ws_head.opcode;
        }
    } while (true);
    stream->close();
    return nullptr;
...
int32_t WSSendMessage(Stream* stream,
    ...
    do {
        ...
        if (ws_head.payload == 126) {
            uint16_t len = size;
            len = ancfl::byteswapOnLittleEndian(len);
            if (stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        } else if (ws_head.payload == 127) {
            uint64_t len = ancfl::byteswapOnLittleEndian(size);
            if (stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        }
