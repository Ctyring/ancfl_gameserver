...
static ancfl::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    ancfl::Config::Lookup("http.request.buffer_size",
                          (uint64_t)(4 * 1024),
                          "http request buffer size");

static ancfl::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    ancfl::Config::Lookup("http.request.max_body_size",
                          (uint64_t)(64 * 1024 * 1024),
                          "http request max body size");
static ancfl::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    ancfl::Config::Lookup("http.response.buffer_size",
                          (uint64_t)(4 * 1024),
                          "http response buffer size");

static ancfl::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    ancfl::Config::Lookup("http.response.max_body_size",
                          (uint64_t)(64 * 1024 * 1024),
...
void on_request_http_field(void* data,
    ...
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if (flen == 0) {
        ANCFL_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
HttpRequestParser::HttpRequestParser() : m_error(0) {
    m_data.reset(new ancfl::http::HttpRequest);
...
// 1: 成功
//-1: 有错�?//>0: 已处理的字节数，且data有效数据为len - v;
...
void on_response_http_field(void* data,
    ...
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    if (flen == 0) {
        ANCFL_LOG_WARN(g_logger) << "invalid http response field length == 0";
        parser->setError(1002);
        return;
    }
    parser->getData()->setHeader(std::string(field, flen),
...
HttpResponseParser::HttpResponseParser() : m_error(0) {
    m_data.reset(new ancfl::http::HttpResponse);
