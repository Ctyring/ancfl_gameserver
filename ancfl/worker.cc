...
static ancfl::ConfigVar<
    ...
    g_worker_config = ancfl::Config::Lookup(
        "workers",
        std::map<std::string, std::map<std::string, std::string> >(),
        "worker config");

WorkerGroup::WorkerGroup(uint32_t batch_size, ancfl::Scheduler* s)
...
bool WorkerManager::init(
    ...
    for (auto& i : v) {
        ...
        int32_t thread_num = ancfl::GetParamValue(i.second, "thread_num", 1);
        int32_t worker_num = ancfl::GetParamValue(i.second, "worker_num", 1);
