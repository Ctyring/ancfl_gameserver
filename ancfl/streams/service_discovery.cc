...
ServiceItemInfo::ptr ServiceItemInfo::Create(const std::string& ip_and_port,
    ...
    auto port = ancfl::TypeUtil::Atoi(ip_and_port.substr(pos + 1));
...
void ZKServiceDiscovery::start() {
    ...
    m_client.reset(new ancfl::ZKClient);
    bool b =
    ...
    if (!b) {
        ANCFL_LOG_ERROR(g_logger) << "ZKClient init fail, hosts=" << m_hosts;
    }
    m_timer = ancfl::IOManager::GetThis()->addTimer(
        60 * 1000,
        [self, this]() {
            m_isOnTimer = true;
...
void ZKServiceDiscovery::onZKConnect(const std::string& path,
    ...
    if (!ok) {
        ANCFL_LOG_ERROR(g_logger) << "onZKConnect register fail";
    }
    ...
    if (!ok) {
        ANCFL_LOG_ERROR(g_logger) << "onZKConnect query fail";
    }
    ...
    if (!ok) {
        ANCFL_LOG_ERROR(g_logger) << "onZKConnect queryData fail";
    }
}
...
bool ZKServiceDiscovery::existsOrCreate(const std::string& path) {
    ...
    if (v == ZOK) {
        ...
        if (pos == std::string::npos) {
            ANCFL_LOG_ERROR(g_logger) << "existsOrCreate invalid path=" << path;
            return false;
        }
...
static std::string GetProvidersPath(const std::string& domain,
                                    const std::string& service) {
    return "/ancfl/" + domain + "/" + service + "/providers";
}

static std::string GetConsumersPath(const std::string& domain,
                                    const std::string& service) {
    return "/ancfl/" + domain + "/" + service + "/consumers";
}
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain,
    ...
    if (!v) {
        ANCFL_LOG_ERROR(g_logger) << "create path=" << path << " fail";
        return false;
    }
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain,
    ...
    if (service != "all") {
        ...
        if (m_selfInfo.empty()) {
            ANCFL_LOG_ERROR(g_logger) << "queryInfo selfInfo is null";
            return false;
        }
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    }
    {
        ...
        if (it == m_queryInfos.end()) {
            ANCFL_LOG_ERROR(g_logger) << "get_children path=" << path
        ...
        if (it->second.count(service) == 0 && it->second.count("all") == 0) {
            ANCFL_LOG_ERROR(g_logger)
                ...
                << " not exists "
                << ancfl::Join(it->second.begin(), it->second.end(), ",");
            return false;
        }
    }
...
void ZKServiceDiscovery::onZKChanged(const std::string& path,
                                     ZKClient::ptr client) {
    ANCFL_LOG_INFO(g_logger) << "onZKChanged path=" << path;
}

void ZKServiceDiscovery::onZKDeleted(const std::string& path,
                                     ZKClient::ptr client) {
    ANCFL_LOG_INFO(g_logger) << "onZKDeleted path=" << path;
}

void ZKServiceDiscovery::onZKExpiredSession(const std::string& path,
                                            ZKClient::ptr client) {
    ANCFL_LOG_INFO(g_logger) << "onZKExpiredSession path=" << path;
    client->reconnect();
}
