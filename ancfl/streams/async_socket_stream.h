#ifndef __ANCFL_STREAMS_ASYNC_SOCKET_STREAM_H__
#define __ANCFL_STREAMS_ASYNC_SOCKET_STREAM_H__
...
class AsyncSocketStream
   ...
   public:
    void setWorker(ancfl::IOManager* v) { m_worker = v; }
    ancfl::IOManager* getWorker() const { return m_worker; }

    void setIOManager(ancfl::IOManager* v) { m_iomanager = v; }
    ancfl::IOManager* getIOManager() const { return m_iomanager; }
   ...
   protected:
    ancfl::FiberSemaphore m_sem;
    ancfl::FiberSemaphore m_waitSem;
    ...
    // io读写调度
    ancfl::IOManager* m_iomanager;
    // 业务处理调度(来自server分配)
    ancfl::IOManager* m_worker;
