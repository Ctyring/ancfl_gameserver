...
void AsyncSocketStream::Ctx::doRsp() {
    ...
    if (!ancfl::Atomic::compareAndSwapBool(scheduler, scd,
                                           (Scheduler*)nullptr)) {
        return;
    }
    if (!scd || !fiber) {
...
bool AsyncSocketStream::start() {
    if (!m_iomanager) {
        m_iomanager = ancfl::IOManager::GetThis();
    }

    if (!m_worker) {
        m_worker = ancfl::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead() {
    ...
    ANCFL_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    ANCFL_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    ANCFL_ASSERT(m_iomanager == ancfl::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get() {
    ...
    for (uint32_t i = 0; i < m_size; ++i) {
        auto idx = ancfl::Atomic::addFetch(m_idx, 1);
