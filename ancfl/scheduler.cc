...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string& name)
    ...
    if (use_caller) {
        ...
        // 设置当前线程的协程调度器
        t_scheduler = this;
        // 设置当前线程的协�?        m_rootFiber.reset(new
        // Fiber(std::bind(&Scheduler::run, this), 0, true));
...
void Scheduler::stop() {
    ...
    if (m_rootFiber && m_threadCount == 0 &&
        ...
        ANCFL_LOG_INFO(g_logger) << this << " stopped";
        m_stopping = true;
...
void Scheduler::run() {
    // ANCFL_LOG_INFO(g_logger) << "run";
    set_hook_enable(true);
    setThis();
    if (ancfl::GetThreadId() != m_rootThread) {
    ...
    if (tickle_me) {
        // ANCFL_LOG_DEBUG(g_logger) << "唤醒其他线程";
        tickle();
    }

    // 判断协程状态是否合�?        if (ft.fiber && (ft.fiber->getState() !=
    // Fiber::TERM &&
    ...
    // 检查调度状态，如果是拿到了无效的协程数据，就继续调度�?            if
    // (is_active) {
    --m_activeThreadCount;
    continue;
}
// 如果是真的没任务了，就切换到idle协程�?            if (idle_fiber->getState()
// == Fiber::TERM) { 如果连idle协程都退出了，就结束了，退出死循环
...
void Scheduler::tickle() {
    ANCFL_LOG_INFO(g_logger) << "tickle";
}
...
void Scheduler::switchTo(int thread) {
    ANCFL_ASSERT(Scheduler::GetThis() != nullptr);
    if (Scheduler::GetThis() == this) {
        if (thread == -1 || thread == ancfl::GetThreadId()) {
            return;
        }
    }
