#ifndef __ANCFL_DB_REDIS_H__
#define __ANCFL_DB_REDIS_H__
...
class FoxRedis : public IRedis {
   ...
   private:
    struct FCtx {
        std::string cmd;
        ancfl::Scheduler* scheduler;
        ancfl::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
class FoxRedisCluster : public IRedis {
   public:
    ...
    FoxRedisCluster(ancfl::FoxThread* thr,
                    const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef ancfl::Singleton<RedisManager> RedisMgr;
