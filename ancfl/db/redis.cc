...
Redis::Redis(const std::map<std::string, std::string>& conf) {
    ...
    m_port = ancfl::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = ancfl::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if (tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = ancfl::TypeUtil::Atoi(tmp);
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            ...
            if (!r) {
                ANCFL_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":"
                                          << m_port << ", " << m_name << ")";
                return false;
            }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
    ...
    if (!r) {
        if (m_logEnable) {
            ANCFL_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host
                                      << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
...
ReplyPtr Redis::getReply() {
    ...
    if (m_logEnable) {
        ANCFL_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":"
                                  << m_port << ")(" << m_name << ")";
    }
    return nullptr;
}
...
FoxRedis::FoxRedis(ancfl::FoxThread* thr,
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if (!r) {
        ANCFL_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":"
                                  << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        ANCFL_LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host
        ...
        if (!ar->m_passwd.empty()) {
            ...
            if (rt) {
                ANCFL_LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
            }
        }
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    ANCFL_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb " << c->c.tcp.host
...
void FoxRedis::CmdCb(redisAsyncContext* ac, void* r, void* privdata) {
    ...
    if (ctx->timeout) {
        delete ctx;
        // if(ctx && ctx->fiber) {
        //     ANCFL_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "'
        //     timeout("
...
//     ANCFL_LOG_INFO(g_logger) << "DelayTimeCb";
...
bool FoxRedis::init() {
    if (m_thread == ancfl::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit() {
    // ANCFL_LOG_INFO(g_logger) << "pinit m_status=" << m_status;
    ...
    m_context.reset(ctx, ancfl::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char* fmt, va_list ap) {
    ...
    if (len == -1) {
        ANCFL_LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
        return nullptr;
    }
    ...
    //     free(buf);
    // }
    // ctx->scheduler = ancfl::Scheduler::GetThis();
    // ctx->fiber = ancfl::Fiber::GetThis();
    // ctx->thread = m_thread;

    FCtx fctx;
    fctx.cmd.append(buf, len);
    free(buf);
    fctx.scheduler = ancfl::Scheduler::GetThis();
    fctx.fiber = ancfl::Fiber::GetThis();
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
    ...
    do {
        ...
        int len = redisFormatCommandArgv(&buf, argv.size(), &(args[0]),
                                         &(args_len[0]));
        if (len == -1 || !buf) {
            ANCFL_LOG_ERROR(g_logger) << "redis fmt error";
            return nullptr;
        }
        fctx.cmd.append(buf, len);
        free(buf);
...
void FoxRedis::pcmd(FCtx* fctx) {
    if (m_status == UNCONNECTED) {
        ANCFL_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port
...
FoxRedis::Ctx::Ctx(FoxRedis* r)
    : ev(nullptr),
      ...
      ,
      thread(nullptr) {
    ancfl::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx() {
    // cancelEvent();
    ANCFL_ASSERT(thread == ancfl::FoxThread::GetThis());
    // ANCFL_ASSERT(destory == 0);
    ancfl::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    // cancelEvent();
    if (ev) {
        evtimer_del(ev);
...
void FoxRedis::Ctx::cancelEvent() {
    // if(ev) {
    //     if(thread == ancfl::FoxThread::GetThis()) {
    //         evtimer_del(ev);
    //         event_free(ev);
    //     } else {
    //         auto e = ev;
...
FoxRedisCluster::FoxRedisCluster(ancfl::FoxThread* thr,
                                 const std::map<std::string, std::string>& conf)
    : m_thread(thr), m_status(UNCONNECTED), m_event(nullptr) {
    m_ctxCount = 0;
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c,
    ...
    if (r->type != REDIS_REPLY_STATUS) {
        ANCFL_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                                  << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (!r->str) {
        ANCFL_LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host
                                  << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        ANCFL_LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host
                                 << ", " << fr->m_name << ")";
    } else {
        ANCFL_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                                  << fr->m_host << ", " << fr->m_name << ")";
    }
}
...
bool FoxRedisCluster::pinit() {
    ...
    ANCFL_LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
    ...
    m_status = CONNECTED;
    // m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, ancfl::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::Ctx::cancelEvent() {
    // ANCFL_LOG_INFO(g_logger) << "cancelEvent " << ancfl::FoxThread::GetThis()
    //            << " - " << thread
    //            << " - " << ancfl::IOManager::GetThis()
    //            << " - " << cancel_count;
    // if(thread != ancfl::FoxThread::GetThis()) {
    //     ANCFL_LOG_INFO(g_logger) << "cancelEvent " <<
    //     ancfl::FoxThread::GetThis()
    //                << " - " << thread
    //                << " - " << ancfl::IOManager::GetThis()
    //                << " - " << cancel_count;

    //    //ANCFL_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " "
    //    << thread->getId()
    //    //           << " this=" << ancfl::FoxThread::GetThis();
    //    //ANCFL_ASSERT(thread == ancfl::FoxThread::GetThis());
    //}
    // ANCFL_ASSERT(!ancfl::IOManager::GetThis());
    ////if(ancfl::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////ANCFL_ASSERT(!ancfl::Fiber::GetThis());
    ////ancfl::RWMutex::WriteLock lock(mutex);
    // if(++cancel_count > 1) {
    //     return;
    // }
    // if(ev) {
...
IRedis::ptr RedisManager::get(const std::string& name) {
    ...
    if (r->getType() == IRedis::FOX_REDIS ||
        ...
        return std::shared_ptr<IRedis>(r, ancfl::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init() {
    ...
    for (auto& i : m_config) {
        ...
        auto pool = ancfl::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for (int n = 0; n < pool; ++n) {
            if (type == "redis") {
                ancfl::Redis* rds(new ancfl::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                ancfl::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ancfl::Atomic::addFetch(done, 1);
            } else if (type == "redis_cluster") {
                ancfl::RedisCluster* rds(new ancfl::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                ancfl::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                ancfl::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        ancfl::FoxRedis* rds(new ancfl::FoxRedis(
                            ancfl::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
                ...
                ancfl::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        ancfl::FoxRedisCluster* rds(new ancfl::FoxRedisCluster(
                            ancfl::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
