...
static ancfl::ConfigVar<
    ...
    g_mysql_dbs = ancfl::Config::Lookup(
        "mysql.dbs",
...
static MYSQL* mysql_init(std::map<std::string, std::string>& params,
    ...
    if (mysql == nullptr) {
        ANCFL_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = ancfl::GetParamValue(params, "port", 0);
    std::string host = ancfl::GetParamValue<std::string>(params, "host");
    std::string user = ancfl::GetParamValue<std::string>(params, "user");
    std::string passwd = ancfl::GetParamValue<std::string>(params, "passwd");
    std::string dbname = ancfl::GetParamValue<std::string>(params, "dbname");
...
bool MySQL::connect() {
    ...
    m_hasError = false;
    m_poolSize = ancfl::GetParamValue(m_params, "pool", 5);
    m_mysql.reset(m, mysql_close);
    return true;
}

ancfl::IStmt::ptr MySQL::prepare(const std::string& sql) {
...
int MySQL::execute(const char* format, va_list ap) {
    m_cmd = ancfl::StringUtil::Formatv(format, ap);
...
std::shared_ptr<MySQL> MySQL::getMySQL() {
    return MySQL::ptr(this, ancfl::nop<MySQL>);
}
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    if (mysql == nullptr) {
        ANCFL_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if (sql == nullptr) {
        ANCFL_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }
...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    ...
    // return 0;
    return bindString(idx, ancfl::Time2Str(value));
}
...
int64_t MySQLRes::getInt64(int idx) {
    return ancfl::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx) {
    return ancfl::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx) {
    ...
    return ancfl::Str2Time(m_cur[idx]);
}
...
int MySQLTransaction::execute(const std::string& sql) {
    if (m_isFinished) {
        ANCFL_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if (it != m_conns.end()) {
        if (!it->second.empty()) {
            ...
            if (rt->ping()) {
                ...
                ANCFL_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
int MySQLManager::execute(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        ANCFL_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                                  << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        ANCFL_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
                                  << ") fail, format=" << format;
        return nullptr;
    }
