...
static ancfl::ConfigVar<
    ...
    g_sqlite3_dbs = ancfl::Config::Lookup(
        "sqlite3.dbs",
...
int SQLite3Stmt::bindTime(int idx, const time_t& value) {
    return bind(idx, ancfl::Time2Str(value));
}
...
time_t SQLite3Data::getTime(int idx) {
    ...
    return ancfl::Str2Time(str.c_str());
}
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
    ...
    lock.unlock();
    std::string path = ancfl::GetParamValue<std::string>(args, "path");
    if (path.empty()) {
        ANCFL_LOG_ERROR(g_logger) << "open db name=" << name << " path is null";
        return nullptr;
    }

    if (path.find(":") == std::string::npos) {
        path = ancfl::EnvMgr::GetInstance()->getAbsoluteWorkPath(path);
    }
    ...
    std::string sql = ancfl::GetParamValue<std::string>(args, "sql");
    if (!sql.empty()) {
        if (rt->execute(sql)) {
            ANCFL_LOG_ERROR(g_logger)
...
int SQLite3Manager::execute(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        ANCFL_LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name
                                  << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr SQLite3Manager::query(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        ANCFL_LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name
                                  << ") fail, format=" << format;
        return nullptr;
    }
