...
void FoxThread::read_cb(evutil_socket_t sock, short which, void* args) {
    ...
    if (recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        for (auto it = callbacks.begin(); it != callbacks.end(); ++it) {
            if (*it) {
                // ANCFL_ASSERT(thread == GetThis());
                try {
                    (*it)();
                } catch (std::exception& ex) {
                    ANCFL_LOG_ERROR(g_logger) << "exception:" << ex.what();
                } catch (const char* c) {
                    ANCFL_LOG_ERROR(g_logger) << "exception:" << c;
                } catch (...) {
                    ANCFL_LOG_ERROR(g_logger) << "uncatch exception";
                }
            } else {
        ...
        ancfl::Atomic::addFetch(thread->m_total, callbacks.size());
...
FoxThread::FoxThread(const std::string& name, struct event_base* base)
    ...
    if (evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == -1) {
        // ANCFL_LOG_ERROR(g_logger) << "FoxThread init error";
...
void FoxThread::start() {
    if (m_thread) {
        // ANCFL_LOG_ERROR(g_logger) << "FoxThread is running";
...
bool FoxThreadPool::dispatch(callback cb) {
    do {
        ancfl::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if (!m_advance) {
...
bool FoxThreadPool::batchDispatch(const std::vector<callback>& cbs) {
    ancfl::Atomic::addFetch(m_total, cbs.size());
    RWMutex::WriteLock lock(m_mutex);
    if (!m_advance) {
        for (auto cb : cbs) {
...
const std::string& FoxThread::GetFoxThreadName() {
    ...
    uint64_t tid = ancfl::GetThreadId();
    do {
...
void FoxThread::setThis() {
    m_name = m_name + "_" + std::to_string(ancfl::GetThreadId());
    s_thread = this;

    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names[ancfl::GetThreadId()] = m_name;
}

void FoxThread::unsetThis() {
    s_thread = nullptr;
    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names.erase(ancfl::GetThreadId());
}
...
void FoxThreadManager::init() {
    ...
    for (auto i : m) {
        auto num = ancfl::GetParamValue(i.second, "num", 0);
        auto name = i.first;
        auto advance = ancfl::GetParamValue(i.second, "advance", 0);
        if (num <= 0) {
        ...
        if (num == 1) {
            ...
            ANCFL_LOG_INFO(g_logger) << "init thread : " << i.first;
        } else {
            m_threads[i.first] =
...
void FoxThreadManager::start() {
    for (auto i : m_threads) {
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " start begin";
        i.second->start();
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " start end";
    }
}

void FoxThreadManager::stop() {
    for (auto i : m_threads) {
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " stop begin";
        i.second->stop();
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " stop end";
    }
    for (auto i : m_threads) {
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " join begin";
        i.second->join();
        ANCFL_LOG_INFO(g_logger) << "thread: " << i.first << " join end";
    }
}
