...
static ancfl::ConfigVar<uint32_t>::ptr g_daemon_restart_interval =
    ancfl::Config::Lookup("daemon.restart_interval",
                          (uint32_t)5,
...
std::string ProcessInfo::toString() const {
    ...
    ss << "[ProcessInfo parent_id=" << parent_id << " main_id=" << main_id
       << " parent_start_time=" << ancfl::Time2Str(parent_start_time)
       << " main_start_time=" << ancfl::Time2Str(main_start_time)
...
static int real_daemon(int argc,
    ...
    while (true) {
        ...
        if (pid == 0) {
            ...
            if (status) {
                if (status == 9) {
                    ANCFL_LOG_INFO(g_logger) << "killed";
                    break;
                } else {
                    ANCFL_LOG_ERROR(g_logger)
                ...
                ANCFL_LOG_INFO(g_logger) << "child finished pid=" << pid;
                break;
            }
