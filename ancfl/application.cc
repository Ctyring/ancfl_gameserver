...
static ancfl::ConfigVar<std::string>::ptr g_server_work_path =
    ancfl::Config::Lookup("server.work_path",
                          std::string("/apps/work/ancfl"),
                          "server work path");

static ancfl::ConfigVar<std::string>::ptr g_server_pid_file =
    ancfl::Config::Lookup("server.pid_file",
                          std::string("ancfl.pid"),
                          "server pid file");

static ancfl::ConfigVar<std::string>::ptr g_service_discovery_zk =
    ancfl::Config::Lookup("service_discovery.zk",
                          std::string(""),
                          "service discovery zookeeper");

static ancfl::ConfigVar<std::vector<TcpServerConf> >::ptr g_servers_conf =
    ancfl::Config::Lookup("servers",
...
bool Application::init(int argc, char** argv) {
    ...
    ancfl::EnvMgr::GetInstance()->addHelp("c", "conf path default: ./conf");
    ...
    std::string conf_path = ancfl::EnvMgr::GetInstance()->getConfigPath();
    ANCFL_LOG_INFO(g_logger) << "load conf path:" << conf_path;
    ancfl::Config::LoadFromConfDir(conf_path);
    ...
    int run_type = 0;
    if (ancfl::EnvMgr::GetInstance()->has("s")) {
        run_type = 1;
    }
    if (ancfl::EnvMgr::GetInstance()->has("d")) {
        run_type = 2;
    }
    ...
    if (ancfl::FSUtil::IsRunningPidfile(pidfile)) {
        ANCFL_LOG_ERROR(g_logger) << "server is running:" << pidfile;
        return false;
    }

    if (!ancfl::FSUtil::Mkdir(g_server_work_path->getValue())) {
        ANCFL_LOG_FATAL(g_logger)
...
bool Application::run() {
    bool is_daemon = ancfl::EnvMgr::GetInstance()->has("d");
...
int Application::main(int argc, char** argv) {
    // SIGPIPE是一个信号，当在一个已经关闭的套接字上写数据时，内核会向进程发送该信号�?    // 默认情况下，这会导致程序终止�?
    // SIG_IGN是一个特殊的信号处理程序，它表示忽略该信号�?
    // 因此，代�?signal(SIGPIPE,SIG_IGN)"实际上是在告诉系统，在遇到SIGPIPE信号时，应该忽略它，
    // 而不是终止程序。这对于防止程序意外终止很有用，特别是在写入一个网络套接字时，有时该套接字可能已经关闭，但程序并不知道�?    signal(SIGPIPE, SIG_IGN);
    ANCFL_LOG_INFO(g_logger) << "main";
    std::string conf_path = ancfl::EnvMgr::GetInstance()->getConfigPath();
    ancfl::Config::LoadFromConfDir(conf_path, true);
    {
    ...
    m_mainIOManager.reset(new ancfl::IOManager(1, true, "main"));
    m_mainIOManager->schedule(std::bind(&Application::run_fiber, this));
    m_mainIOManager->addTimer(
        2000,
        []() {
            // ANCFL_LOG_INFO(g_logger) << "hello";
        },
        true);
...
int Application::run_fiber() {
    ...
    for (auto& i : modules) {
        if (!i->onLoad()) {
            ANCFL_LOG_ERROR(g_logger) << "module name=" << i->getName()
    ...
    for (auto& i : http_confs) {
        ...
        for (auto& a : i.address) {
            ...
            if (pos == std::string::npos) {
                // ANCFL_LOG_ERROR(g_logger) << "invalid address: " << a;
            ...
            // 127.0.0.1
            auto addr =
                ancfl::IPAddress::Create(a.substr(0, pos).c_str(), port);
            if (addr) {
                address.push_back(addr);
                continue;
            }
            std::vector<std::pair<Address::ptr, uint32_t> > result;
            if (ancfl::Address::GetInterfaceAddresses(result,
                                                      a.substr(0, pos))) {
                for (auto& x : result) {
            ...
            auto aaddr = ancfl::Address::LookupAny(a);
            if (aaddr) {
        ...
        IOManager* accept_worker = ancfl::IOManager::GetThis();
        IOManager* io_worker = ancfl::IOManager::GetThis();
        IOManager* process_worker = ancfl::IOManager::GetThis();
        if (!i.accept_worker.empty()) {
            accept_worker = ancfl::WorkerMgr::GetInstance()
        ...
        if (!i.io_worker.empty()) {
            io_worker = ancfl::WorkerMgr::GetInstance()
        ...
        if (!i.process_worker.empty()) {
            process_worker = ancfl::WorkerMgr::GetInstance()
        ...
        if (i.type == "http") {
            server.reset(new ancfl::http::HttpServer(
                i.keepalive, process_worker, io_worker, accept_worker));
        } else if (i.type == "ws") {
            server.reset(new ancfl::http::WSServer(process_worker, io_worker,
                                                   accept_worker));
        } else if (i.type == "rock") {
            server.reset(new ancfl::RockServer("rock", process_worker,
                                               io_worker, accept_worker));
        } else if (i.type == "nameserver") {
            server.reset(new ancfl::RockServer("nameserver", process_worker,
                                               io_worker, accept_worker));
        } else {
            ANCFL_LOG_ERROR(g_logger)
        ...
        if (!server->bind(address, fails, i.ssl)) {
            for (auto& x : fails) {
                ANCFL_LOG_ERROR(g_logger) << "bind address fail:" << *x;
            }
            _exit(0);
        }
        if (i.ssl) {
    ...
    if (!g_service_discovery_zk->getValue().empty()) {
        ...
        if (!getServer("http", svrs)) {
            m_serviceDiscovery->setSelfInfo(ancfl::GetIPv4() +
                                            ":0:" + ancfl::GetHostName());
        } else {
