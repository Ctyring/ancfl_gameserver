#ifndef __ANCFL_WORKER_H__
#define __ANCFL_WORKER_H__
...
class WorkerGroup : Noncopyable,
   ...
   public:
    ...
    static WorkerGroup::ptr Create(
        uint32_t batch_size,
        ancfl::Scheduler* s = ancfl::Scheduler::GetThis()) {
   ...
   public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if (s) {
            ...
            static ancfl::Logger::ptr s_logger = ANCFL_LOG_NAME("system");
            ANCFL_LOG_ERROR(s_logger)
...
typedef ancfl::Singleton<WorkerManager> WorkerMgr;
