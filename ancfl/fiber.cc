...
static Logger::ptr g_logger = ANCFL_LOG_NAME("system");
...
Fiber::Fiber() {
    ...
    ANCFL_LOG_DEBUG(g_logger) << "Fiber::Fiber Main";
}
...
void Fiber::call() {
    SetThis(this);
    m_state = EXEC;
    ANCFL_LOG_ERROR(g_logger) << getId();
...
void Fiber::MainFunc() {
    ...
    try {
        ...
        cur->m_state = EXCEPT;
        ANCFL_LOG_ERROR(g_logger) << "Fiber Except: " << ex.what()
                                  << " fiber_id=" << cur->getId() << std::endl
                                  << ancfl::BacktraceToString();
    } catch (...) {
        cur->m_state = EXCEPT;
        ANCFL_LOG_ERROR(g_logger) << "Fiber Except"
                                  << " fiber_id=" << cur->getId() << std::endl
                                  << ancfl::BacktraceToString();
    }
