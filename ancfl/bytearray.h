 ...
 * @brief 二进制数�?序列�?反序列化)
 */
#ifndef __ANCFL_BYTEARRAY_H__
#define __ANCFL_BYTEARRAY_H__
...
class ByteArray {
   public:
    ...
    struct Node {
        /**
         * @brief 构造指定大小的内存�?         * @param[in] s 内存块字节数
         */
        Node(size_t s);
    ...
    /**
     * @brief 写入固定长度int16_t类型的数�?大端/小端)
     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    void writeFint16(int16_t value);
    /**
     * @brief 写入固定长度uint16_t类型的数�?大端/小端)
     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    ...
    /**
     * @brief 写入固定长度uint32_t类型的数�?大端/小端)
     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    ...
    /**
     * @brief 写入固定长度uint64_t类型的数�?大端/小端)
     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    ...
    /**
     * @brief 写入float类型的数�?     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    void writeFloat(float value);

    /**
     * @brief 写入double类型的数�?     * @post m_position += sizeof(value)
     *       如果m_position > m_size �?m_size = m_position
     */
    void writeDouble(double value);

    /**
     * @brief 写入std::string类型的数�?用uint16_t作为长度类型
    ...
    /**
     * @brief 写入std::string类型的数�?用uint32_t作为长度类型
    ...
    /**
     * @brief 写入std::string类型的数�?用uint64_t作为长度类型
    ...
    /**
     * @brief 写入std::string类型的数�?用无符号Varint64作为长度类型
    ...
    /**
     * @brief 读取int8_t类型的数�?     * @pre getReadSize() >= sizeof(int8_t)
    ...
    /**
     * @brief 读取uint8_t类型的数�?     * @pre getReadSize() >= sizeof(uint8_t)
    ...
    /**
     * @brief 读取int16_t类型的数�?     * @pre getReadSize() >= sizeof(int16_t)
    ...
    /**
     * @brief 读取int32_t类型的数�?     * @pre getReadSize() >= sizeof(int32_t)
    ...
    /**
     * @brief 读取int64_t类型的数�?     * @pre getReadSize() >= sizeof(int64_t)
    ...
    /**
     * @brief 读取float类型的数�?     * @pre getReadSize() >= sizeof(float)
    ...
    /**
     * @brief 读取double类型的数�?     * @pre getReadSize() >= sizeof(double)
    ...
    /**
     * @brief 读取std::string类型的数�?用uint16_t作为长度
    ...
    /**
     * @brief 读取std::string类型的数�?用uint32_t作为长度
    ...
    /**
     * @brief 读取std::string类型的数�?用uint64_t作为长度
    ...
    /**
     * @brief 读取std::string类型的数�?用无符号Varint64作为长度
    ...
    /**
     * @brief 写入size长度的数�?     * @param[in] buf 内存缓存指针
     * @param[in] size 数据大小
     * @post m_position += size, 如果m_position > m_size �?m_size = m_position
     */
     ...
     * @param[in] size 数据大小
     * @post m_position += size, 如果m_position > m_size �?m_size = m_position
     * @exception 如果getReadSize() < size 则抛�?std::out_of_range
     */
     ...
     * @post 如果m_position > m_size �?m_size = m_position
     * @exception 如果m_position > m_capacity 则抛�?std::out_of_range
     */
    void setPosition(size_t v);

    /**
     * @brief 把ByteArray的数据写入到文件�?     * @param[in] name 文件�?     */
   ...
   private:
    ...
    /// 当前操作位置
    size_t m_position;
    /// 当前的总容�?    size_t m_capacity;
    /// 当前数据的大�?    size_t m_size;
    /// 字节�?默认大端
    int8_t m_endian;
    /// 第一个内存块指针
    Node* m_root;
