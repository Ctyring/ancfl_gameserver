...
bool SceneModule::GetObject(int32_t scene_id,
                            uint64_t object_id,
                            SceneObject& object) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool SceneModule::GetSceneObjects(int32_t scene_id,
...
bool SceneModule::GetScenePlayers(int32_t scene_id,
    ...
    for (const auto& pair : it->second) {
        if (pair.second.type == SceneObjectType::PLAYER) {
            players.push_back(
                static_cast<const PlayerSceneObject&>(pair.second));
        }
    }
...
bool SceneModule::GetSceneMonsters(int32_t scene_id,
    ...
    for (const auto& pair : it->second) {
        if (pair.second.type == SceneObjectType::MONSTER) {
            monsters.push_back(
                static_cast<const MonsterSceneObject&>(pair.second));
        }
    }
...
bool SceneModule::UpdatePlayerPosition(uint64_t role_id,
                                       float x,
                                       float y,
                                       float z,
                                       float rotation_y) {
...
bool SceneModule::GetPlayerPosition(uint64_t role_id,
                                    float& x,
                                    float& y,
                                    float& z,
                                    float& rotation_y) {
...
bool SceneModule::GetVisibleObjects(uint64_t role_id,
                                    std::vector<SceneObject>& objects) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
    ...
    for (auto& scene_pair : scene_objects_) {
        for (auto& obj_pair : scene_pair.second) {
            if (obj_pair.second.type == SceneObjectType::PLAYER) {
                PlayerSceneObject& player =
                    static_cast<PlayerSceneObject&>(obj_pair.second);
                ...
                if (player.is_moving) {
                    ...
                    if (distance < 0.1f) {
                        ...
                        // 继续移动
                        float move_distance =
                            player.speed * 0.1f;  // 假设每100ms更新一次
