...
bool EquipModule::GetEquipList(uint64_t role_id,
...
bool EquipModule::GetEquipInfo(uint64_t role_id,
                               uint64_t equip_id,
                               EquipInfo& info) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::WearEquip(uint64_t role_id,
                            uint64_t equip_id,
                            int32_t position) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::GetWornEquips(uint64_t role_id,
                                std::vector<EquipInfo>& equips) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::StrengthenEquip(uint64_t role_id,
                                  uint64_t equip_id,
                                  int32_t level) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::GetStrengthenLevel(uint64_t role_id,
                                     uint64_t equip_id,
                                     int32_t& level) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::UpgradeStar(uint64_t role_id,
                              uint64_t equip_id,
                              int32_t star_level) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::GetStarLevel(uint64_t role_id,
                               uint64_t equip_id,
                               int32_t& star_level) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::InlayGem(uint64_t role_id,
                           uint64_t equip_id,
                           int32_t gem_config_id,
                           int32_t gem_index) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::RemoveGem(uint64_t role_id,
                            uint64_t equip_id,
                            int32_t gem_index) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::GetGems(uint64_t role_id,
                          uint64_t equip_id,
...
bool EquipModule::CalculateEquipAttribute(uint64_t role_id,
                                          EquipAttribute& attribute) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EquipModule::CalculateSingleEquipAttribute(const EquipInfo& equip,
                                                EquipAttribute& attribute) {
    // TODO: 根据装备配置计算属性
    // 这里使用简单的计算公式
    ...
    attribute.mp = 0;
    attribute.attack =
        base_attack * strengthen_bonus * star_bonus * quality_bonus;
    attribute.defense =
        base_defense * strengthen_bonus * star_bonus * quality_bonus;
    attribute.speed = 0;
