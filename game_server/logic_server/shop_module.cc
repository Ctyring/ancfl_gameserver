...
bool ShopModule::BuyItem(uint64_t role_id,
                         int32_t shop_id,
    ...
    if (!DeductMoney(role_id, item.price_type, total_price)) {
        LOG_ERROR("Not enough money: role_id=%llu, price_type=%d, price=%d",
                  role_id, item.price_type, total_price);
        return false;
    }

    // 添加物品到背包
    // TODO: 调用背包模块添加物品
    LOG_INFO("Item added to bag: role_id=%llu, item_config_id=%d, count=%d",
             role_id, item_config_id, count);
    ...
    LOG_INFO(
        "Item bought: role_id=%llu, shop_id=%d, item_config_id=%d, count=%d, "
        "price=%d",
        role_id, shop_id, item_config_id, count, total_price);
    return true;
}

bool ShopModule::CanBuyItem(uint64_t role_id,
                            int32_t shop_id,
                            int32_t item_config_id,
                            int32_t count) {
    // 获取商品信息
    ShopItem item;
    if (!GetShopItem(shop_id, item_config_id, item)) {
        LOG_ERROR("Shop item not found: shop_id=%d, item_config_id=%d", shop_id,
                  item_config_id);
        return false;
    }
    ...
    if (bought_count + count > item.limit_count && item.limit_count > 0) {
        LOG_ERROR(
            "Buy limit reached: role_id=%llu, bought=%d, want=%d, limit=%d",
    ...
    if (!CheckMoneyEnough(role_id, item.price_type, total_price)) {
        LOG_ERROR("Not enough money: role_id=%llu, price_type=%d, need=%d",
                  role_id, item.price_type, total_price);
        return false;
    }
...
bool ShopModule::GetItemBuyCount(uint64_t role_id,
                                 int32_t shop_id,
                                 int32_t item_config_id,
                                 int32_t& count) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool ShopModule::AddItemBuyCount(uint64_t role_id,
                                 int32_t shop_id,
                                 int32_t item_config_id,
                                 int32_t count) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool ShopModule::OpenMysteryShop(uint64_t role_id) {
    ...
    LOG_INFO("Mystery shop opened: role_id=%llu, expire_time=%lld", role_id,
             info.expire_time);
    return true;
}
...
bool ShopModule::BuyMysteryItem(uint64_t role_id, int32_t slot_index) {
    ...
    for (auto& item : it->second.items) {
        if (item.slot_index == slot_index) {
            if (item.is_bought) {
                LOG_ERROR(
                    "Mystery item already bought: role_id=%llu, slot_index=%d",
                    role_id, slot_index);
                return false;
            }
            ...
            if (!DeductMoney(role_id, item.price_type, price)) {
                LOG_ERROR(
                    "Not enough money for mystery item: role_id=%llu, price=%d",
                    role_id, price);
                return false;
            }
            ...
            // TODO: 调用背包模块添加物品
            LOG_INFO(
                "Mystery item added to bag: role_id=%llu, item_config_id=%d",
                role_id, item.item_config_id);

            // 标记为已购买
            item.is_bought = true;

            LOG_INFO(
                "Mystery item bought: role_id=%llu, slot_index=%d, "
                "item_config_id=%d, price=%d",
                role_id, slot_index, item.item_config_id, price);
            return true;
        }
    }

    LOG_ERROR("Mystery item not found: role_id=%llu, slot_index=%d", role_id,
              slot_index);
    return false;
}
...
bool ShopModule::GetPurchaseRecords(uint64_t role_id,
...
bool ShopModule::AddPurchaseRecord(uint64_t role_id,
...
bool ShopModule::CheckMoneyEnough(uint64_t role_id,
                                  int32_t price_type,
                                  int32_t price) {
    // TODO: 检查货币是否足够
    return true;
...
bool ShopModule::DeductMoney(uint64_t role_id,
                             int32_t price_type,
                             int32_t price) {
    // TODO: 扣除货币
    return true;
...
void ShopModule::OnTimer() {
    ...
    while (it != mystery_shop_cache_.end()) {
        if (now > it->second.expire_time) {
            LOG_INFO("Mystery shop expired and removed: role_id=%llu",
                     it->first);
...
bool ShopModule::GetShopItem(int32_t shop_id,
                             int32_t item_config_id,
                             ShopItem& item) {
    auto it = shop_configs_.find(shop_id);
    if (it == shop_configs_.end()) {
...
bool ShopModule::LoadShopConfigs() {
    ...
    if (!g_config_manager->HasConfig("shop_config")) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
            "Shop config table not found in database, please run "
            "insert_config_data.sql first");
        return false;
    }

    std::vector<ConfigRow> rows;
    if (!g_config_manager->GetConfigRows("shop_config", rows)) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())("Failed to load shop config rows");
        return false;
    }
    ...
    for (const auto& row : rows) {
        ...
        shop.shop_id = config_id;
        shop.shop_name =
            CONFIG_GET_STRING("shop_config", config_id, "name", "");
        shop.type = static_cast<ShopType>(
            CONFIG_GET_INT("shop_config", config_id, "type", 1));
        shop.refresh_time =
            CONFIG_GET_INT("shop_config", config_id, "refresh_time", 0);
        shop.refresh_cost =
            CONFIG_GET_INT("shop_config", config_id, "refresh_cost", 0);
        shop.is_open =
            CONFIG_GET_BOOL("shop_config", config_id, "is_open", true);
        shop.items.clear();

        shop_configs_[shop.shop_id] = shop;
    }

    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())("Loaded %d shop configs from database",
                                     shop_configs_.size());
    return true;
}
