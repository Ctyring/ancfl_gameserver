...
bool RoleModule::UpdateRoleInfo(const RoleData& info) {
    ...
    req.set_战功(info.战功);
    req.set_成就(info.成就);
    req.set_战斗力(info.战斗力);
    req.set_当前场景(info.当前场景);
...
bool RoleModule::SaveRoleData(uint64_t role_id) {
    ...
    req.set_体力(it->second.体力);
    req.set_精力(it->second.精力);
    req.set_声望(it->second.声望);
    req.set_荣誉(it->second.荣誉);
    req.set_战功(it->second.战功);
    req.set_成就(it->second.成就);
    req.set_战斗力(it->second.战斗力);
    req.set_当前场景(it->second.当前场景);
