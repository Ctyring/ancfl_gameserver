...
bool TaskModule::InitTasks(uint64_t role_id) {
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())("Tasks initialized: role_id=%llu",
                                     role_id);
    return true;
}
...
bool TaskModule::GetTaskInfo(uint64_t role_id,
...
bool TaskModule::AcceptTask(uint64_t role_id, int32_t task_config_id) {
    ...
    if (!GetTaskConfig(task_config_id, config)) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
            "Task config not found: task_config_id=%d", task_config_id);
        return false;
    }
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
        "Task accepted: role_id=%llu, task_config_id=%d, task_id=%llu", role_id,
        task_config_id, task.task_id);
    return true;
}
...
bool TaskModule::CanAcceptTask(uint64_t role_id, int32_t task_config_id) {
        ...
        for (const auto& task : it->second) {
            if (task.task_config_id == task_config_id) {
                if (task.status == TaskStatus::ACCEPTED ||
                    ...
                    ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
                        "Task already accepted or completed: role_id=%llu, "
                        "task_config_id=%d",
                        role_id, task_config_id);
                    return false;
                }
            }
        }
    ...
    if (config.pre_task_id > 0) {
        if (!CheckPreTask(role_id, config.pre_task_id)) {
            ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
                "Pre task not completed: role_id=%llu, pre_task_id=%d", role_id,
                config.pre_task_id);
            return false;
        }
    }
    ...
    if (config.level_requirement > 0) {
        if (!CheckLevelRequirement(role_id, config.level_requirement)) {
            ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
                "Level requirement not met: role_id=%llu, required_level=%d",
                role_id, config.level_requirement);
            return false;
        }
    }
    ...
    for (auto& task : it->second) {
        if (task.task_config_id == task_config_id &&
            ...
            if (all_completed) {
                ...
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task completed: role_id=%llu, task_config_id=%d", role_id,
                    task_config_id);
            }
...
bool TaskModule::SubmitTask(uint64_t role_id, int32_t task_config_id) {
    ...
    for (auto& task : it->second) {
        if (task.task_config_id == task_config_id &&
            ...
            if (!GiveTaskRewards(role_id, task)) {
                ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
                    "Failed to give task rewards: role_id=%llu, "
                    "task_config_id=%d",
                    role_id, task_config_id);
                return false;
            }
            ...
            ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                "Task submitted: role_id=%llu, task_config_id=%d", role_id,
                task_config_id);
            return true;
        }
    }
    ...
    ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
        "Task not completed or not found: role_id=%llu, task_config_id=%d",
        role_id, task_config_id);
    return false;
}
    ...
    for (const auto& reward : task.rewards) {
        switch (reward.reward_type) {
            case 1:  // 经验
                // TODO: 增加角色经验
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task reward - Exp: role_id=%llu, exp=%d", role_id,
                    reward.reward_count);
                break;
            case 2:  // 金币
                // TODO: 增加角色金币
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task reward - Gold: role_id=%llu, gold=%d", role_id,
                    reward.reward_count);
                break;
            case 3:  // 物品
                // TODO: 添加物品到背包
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task reward - Item: role_id=%llu, item_id=%d, count=%d",
                    role_id, reward.reward_id, reward.reward_count);
                break;
            default:
                break;
        }
...
bool TaskModule::AbandonTask(uint64_t role_id, int32_t task_config_id) {
    ...
    auto task_it =
        std::remove_if(it->second.begin(), it->second.end(),
                       [task_config_id](const TaskInfo& task) {
                           return task.task_config_id == task_config_id &&
                                  task.status == TaskStatus::ACCEPTED;
                       });

    if (task_it == it->second.end()) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
            "Task not found or not in accepted status: role_id=%llu, "
            "task_config_id=%d",
            role_id, task_config_id);
        return false;
    }
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
        "Task abandoned: role_id=%llu, task_config_id=%d", role_id,
        task_config_id);
    return true;
}

bool TaskModule::UpdateTaskProgress(uint64_t role_id,
                                    int32_t task_config_id,
    ...
    for (auto& task : it->second) {
        if (task.task_config_id == task_config_id &&
            ...
            if (condition_index >= 0 &&
                condition_index < task.conditions.size()) {
                ...
                if (all_completed) {
                    ...
                    task.complete_time = time(nullptr);
                    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                        "Task auto completed: role_id=%llu, task_config_id=%d",
                        role_id, task_config_id);
                }
...
bool TaskModule::AddTaskProgress(uint64_t role_id,
    ...
    for (auto& task : it->second) {
        ...
        for (auto& condition : task.conditions) {
            if (condition.condition_type ==
                    static_cast<int32_t>(condition_type) &&
                ...
                updated = true;
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task progress updated: role_id=%llu, task_config_id=%d, "
                    "progress=%d/%d",
                    role_id, task.task_config_id, condition.current_count,
        ...
        if (all_completed) {
            ...
            task.complete_time = time(nullptr);
            ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                "Task completed by progress: role_id=%llu, task_config_id=%d",
                role_id, task.task_config_id);
        }
    }
...
bool TaskModule::RefreshDailyTasks(uint64_t role_id) {
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
        "Daily tasks refreshed: role_id=%llu, count=%d", role_id,
        it->second.size());
    return true;
}

bool TaskModule::GetDailyTasks(uint64_t role_id,
...
bool TaskModule::AcceptDailyTask(uint64_t role_id, int32_t task_config_id) {
    ...
    for (auto& daily_task : it->second) {
        if (daily_task.task_config_id == task_config_id &&
            ...
            ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                "Daily task accepted: role_id=%llu, task_config_id=%d", role_id,
                task_config_id);
            return true;
        }
    }
    ...
    if (it != task_cache_.end()) {
        for (const auto& task : it->second) {
            if (task.task_config_id == task_config_id) {
                ...
                ANCFL_LOG_INFO(ANCFL_LOG_ROOT())(
                    "Task tracked: role_id=%llu, task_config_id=%d", role_id,
                    task_config_id);
                return true;
            }
        }
...
void TaskModule::OnCollectItem(uint64_t role_id,
                               int32_t item_id,
                               int32_t count) {
...
void TaskModule::OnCompleteInstance(uint64_t role_id, int32_t instance_id) {
    AddTaskProgress(role_id, TaskConditionType::COMPLETE_INSTANCE, instance_id,
                    1);
}
...
void TaskModule::OnSkillUpgrade(uint64_t role_id,
                                int32_t skill_id,
                                int32_t level) {
...
bool TaskModule::SaveTaskData(uint64_t role_id) {
    ...
    service_->SendMsgToDBServer(
        static_cast<uint32_t>(MessageID::MSG_TASK_DATA_SYNC_REQ), req);
...
bool TaskModule::LoadTaskConfigs() {
    ...
    if (!g_config_manager->HasConfig("task_config")) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())(
            "Task config table not found in database, please run "
            "insert_config_data.sql first");
        return false;
    }

    std::vector<ConfigRow> rows;
    if (!g_config_manager->GetConfigRows("task_config", rows)) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())("Failed to load task config rows");
        return false;
    }
    ...
    for (const auto& row : rows) {
        ...
        config.task_config_id = config_id;
        config.task_name =
            CONFIG_GET_STRING("task_config", config_id, "name", "");
        config.type = static_cast<TaskType>(
            CONFIG_GET_INT("task_config", config_id, "type", 1));
        config.level_requirement =
            CONFIG_GET_INT("task_config", config_id, "level_requirement", 1);
        config.pre_task_id =
            CONFIG_GET_INT("task_config", config_id, "pre_task_id", 0);
        config.description =
            CONFIG_GET_STRING("task_config", config_id, "description", "");

        task_configs_[config.task_config_id] = config;
    }

    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())("Loaded %d task configs from database",
                                     task_configs_.size());
    return true;
}
