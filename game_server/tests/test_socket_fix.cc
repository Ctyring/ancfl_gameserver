...
void run_client() {
    ...
    // 测试使用 ancfl Socket::recv 方法，直接读取数据
    cout << "[Client] Testing ancfl Socket::recv..." << endl;
    char buffer[1024] = {0};
    
    // 接收消息头
    MessageHeader header;
    int recv_header = client_socket->recv(&header, sizeof(MessageHeader), 0);
    if (recv_header < 0) {
        ...
        int recv_data = client_socket->recv(data, data_len, 0);
        if (recv_data < 0) {
