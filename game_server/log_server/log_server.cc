...
bool LogServer::QueryLogs(const LogQueryCondition& condition,
...
int64_t LogServer::GetLogCount(LogType type,
                               time_t start_time,
                               time_t end_time) {
    // TODO: 从数据库统计日志数量
    return 0;
...
bool LogServer::LogLogin(uint64_t role_id,
...
bool LogServer::LogRecharge(uint64_t role_id,
                            const std::string& role_name,
                            int32_t server_id,
                            int32_t amount,
                            int32_t product_id) {
    ...
    record.extra_data = "amount=" + std::to_string(amount) +
...
bool LogServer::LogConsume(uint64_t role_id,
                           const std::string& role_name,
                           int32_t server_id,
                           int32_t type,
                           int32_t amount,
    ...
    record.extra_data = "type=" + std::to_string(type) +
...
bool LogServer::LogItem(uint64_t role_id,
                        const std::string& role_name,
                        int32_t server_id,
                        int32_t item_id,
                        int32_t count,
    ...
    record.extra_data = "item_id=" + std::to_string(item_id) +
                        ",count=" + std::to_string(count) +
                        ",type=" + std::to_string(type) + ",reason=" + reason;
    record.log_time = time(nullptr);

    return WriteLogAsync(record);
}

bool LogServer::LogTrade(uint64_t role_id,
                         const std::string& role_name,
                         int32_t server_id,
                         uint64_t target_id,
    ...
    record.extra_data = "target_id=" + std::to_string(target_id) +
                        ",target_name=" + target_name + ",items=" + items;
    record.log_time = time(nullptr);

    return WriteLogAsync(record);
}

bool LogServer::LogBattle(uint64_t role_id,
                          const std::string& role_name,
                          int32_t server_id,
                          int32_t battle_type,
                          int32_t result,
    ...
    record.extra_data = "battle_type=" + std::to_string(battle_type) +
...
bool LogServer::WriteToFile(const LogRecord& record) {
    ...
    it->second << "[" << time_buf << "] "
               ...
               << "[role:" << record.role_id << "] " << record.content;
...
std::string LogServer::GetLogFileName(LogType type, time_t log_time) {
    ...
    switch (type) {
        ...
        case LogType::LOGOUT:
            type_name = "logout";
            break;
        case LogType::CREATE_ROLE:
            type_name = "create_role";
            break;
        case LogType::DELETE_ROLE:
            type_name = "delete_role";
            break;
