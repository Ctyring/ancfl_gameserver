...
#define LOG_INFO(msg) do { \
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT()) << "[" << time_str << "] [日志服务器] " << msg; \
} while(0)
...
#define LOG_ERROR(msg) do { \
    ...
    char time_str[20]; \
    strftime(time_str, 20, "%Y-%m-%d %H:%M:%S", tm_info); \
    ANCFL_LOG_ERROR(ANCFL_LOG_ROOT()) << "[" << time_str << "] [日志服务器] 错误: " << msg; \
} while(0)
...
bool LogService::InitService() {
    ...
    try {
        ...
        // 读取日志服务器配置
        if (root["log_server"]) {
            if (root["log_server"]["host"]) {
                ip = root["log_server"]["host"].as<std::string>();
            }
            if (root["log_server"]["port"]) {
                port = root["log_server"]["port"].as<int>();
            }
        }
...
void LogService::UninitService() {
    LOG_INFO("正在反初始化日志服务器...");
    LOG_INFO("日志服务器已反初始化");
}

void LogService::RegisterAllHandlers() {
    // 注册消息处理器（暂时不实现）
    LOG_INFO("消息处理器注册完成");
}

void LogService::OnTimer() {
    // 每秒定时器（暂时不实现）
}

void LogService::OnTimer5s() {
    LOG_INFO("执行5秒定时器回调");
...
bool LogService::ConnectToCenterServer() {
...
bool LogService::RegisterToCenterServer() {
    if (!center_server_conn_) {
    ...
    std::string data = "log_server_register";
...
void LogService::SendHeartbeatToCenterServer() {
    if (!center_server_conn_) {
        LOG_ERROR("中心服务器连接未建立");
        return;
    }
    ...
    std::string data = "log_server_heartbeat";
