...
bool ConfigManager::GetConfigRow(const std::string& table_name,
                                 int32_t id,
                                 ConfigRow& row) {
    std::lock_guard<std::mutex> lock(config_mutex_);
...
bool ConfigManager::GetConfigRows(const std::string& table_name,
...
bool ConfigManager::GetConfigValue(const std::string& table_name,
                                   int32_t id,
...
float ConfigManager::GetConfigFloat(const std::string& table_name,
                                    int32_t id,
                                    const std::string& column,
                                    float default_val) {
    ConfigValue value;
...
std::string ConfigManager::GetConfigString(const std::string& table_name,
                                           int32_t id,
                                           const std::string& column,
...
bool ConfigManager::GetConfigBool(const std::string& table_name,
                                  int32_t id,
                                  const std::string& column,
                                  bool default_val) {
    ConfigValue value;
...
bool ConfigManager::HasConfigRow(const std::string& table_name, int32_t id) {
    ...
    return table_it->second.id_index.find(id) !=
           table_it->second.id_index.end();
}
...
bool ConfigManager::ExecuteQuery(const std::string& sql,
                                 std::vector<ConfigRow>& results) {
    results.clear();
        ...
        for (int i = 0; i < column_count; ++i) {
            ...
            switch (type) {
                ...
                case SQLITE_FLOAT:
                    ...
                    value.float_val =
                        static_cast<float>(sqlite3_column_double(stmt, i));
                    break;
                case SQLITE_TEXT:
                    value.type = ConfigDataType::STRING;
                    value.str_val = reinterpret_cast<const char*>(
...
ConfigValue ConfigManager::ParseValue(const std::string& value,
