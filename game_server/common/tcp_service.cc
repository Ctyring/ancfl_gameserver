...
bool TcpService::Init(const std::string& ip, uint16_t port) {
    ...
    auto addr =
        ancfl::Address::LookupAnyIPAddress(ip + ":" + std::to_string(port));
    if (!addr) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())
            << "Lookup address failed: " << ip << ":" << port;
        return false;
    }
...
void TcpService::handleClient(ancfl::Socket::ptr client) {
    ...
    // 启动接收协程
    ancfl::Fiber::ptr recv_fiber(
        new ancfl::Fiber(std::bind(&TcpService::HandleRecv, this, client)));
    recv_fiber->call();
    ...
    ANCFL_LOG_INFO(ANCFL_LOG_ROOT())
        << "Connection closed, conn_id=" << conn_id;
}

bool TcpService::SendMsgToServer(int32_t server_conn_id,
                                 uint32_t msg_id,
                                 const std::string& data) {
    return SendRawData(server_conn_id, msg_id, 0, 0, data.data(), data.size());
}

bool TcpService::SendMsgToClient(int32_t client_conn_id,
                                 uint32_t msg_id,
                                 const std::string& data) {
...
void TcpService::HandleRecv(ancfl::Socket::ptr client) {
    ...
    while (running_) {
        ...
        if (header.msg_len > 32768 || header.msg_len < sizeof(header)) {
            ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())
                << "Invalid message length: " << header.msg_len;
            break;
        }
...
bool TcpService::ParseMessage(const char* data,
                              uint32_t len,
                              NetPacket& packet) {
    auto msg = CreateMessage(packet.msg_id);
    if (!msg) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())
            << "Unknown message id: " << packet.msg_id;
        return false;
    }

    if (!msg->ParseFromArray(data, len)) {
        ANCFL_LOG_ERROR(ANCFL_LOG_ROOT())
            << "Parse message failed, id: " << packet.msg_id;
        return false;
    }
...
std::shared_ptr<google::protobuf::Message> TcpService::CreateMessage(
    uint32_t msg_id) {
    // 子类需要实现此方法，根据msg_id创建对应的Protobuf消息
    return nullptr;
}

bool TcpService::SendMessage(int32_t conn_id,
                             uint32_t msg_id,
                             uint64_t target_id,
                             uint32_t user_data,
    ...
    return SendRawData(conn_id, msg_id, target_id, user_data, body_data.data(),
                       body_data.size());
}

bool TcpService::SendRawData(int32_t conn_id,
                             uint32_t msg_id,
                             uint64_t target_id,
                             uint32_t user_data,
                             const char* data,
...
void TcpService::Update() {
    ...
    for (auto conn_id : timeout_conns) {
        ANCFL_LOG_WARN(ANCFL_LOG_ROOT())
            << "Connection heart timeout, conn_id=" << conn_id;
