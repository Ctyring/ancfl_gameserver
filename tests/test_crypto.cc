...
void test_aes256ecb() {
    for (int i = 0; i < 4097; ++i) {
        auto str = ancfl::random_string(i);
        std::string encode;
        encode.resize(i + 30);
        std::string decode;
        decode.resize(i + 30);

        auto key = ancfl::random_string(32);

        int32_t len = ancfl::CryptoUtil::AES256Ecb(
            key.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if (len > 0) {
            encode.resize(len);
        }
        len = ancfl::CryptoUtil::AES256Ecb(key.c_str(), encode.c_str(),
...
void test_aes256cbc() {
    for (int i = 0; i < 4097; ++i) {
        ...
        auto key = ancfl::random_string(32);
        auto iv = ancfl::random_string(16);

        int32_t len = ancfl::CryptoUtil::AES256Cbc(
            key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if (len > 0) {
            encode.resize(len);
        }
        len = ancfl::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(),
...
void test_one() {
    // std::string key = ancfl::random_string(32);
    // std::string iv = ancfl::random_string(16);
    // std::string str = ancfl::random_string(125);
    ...
    int32_t len = ancfl::CryptoUtil::AES128Cbc(
        key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
    if (len >= 0) {
        encode.resize(len);
    }
    ...
    int32_t len2 =
        ancfl::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(),
                                     encode.size(), &decode[0], false);
    if (len2 >= 0) {
        decode.resize(len2);
    }
    ...
    std::cout << "encode: " << ancfl::base64encode(encode)
...
void test_rsa() {
    ...
    // std::cout << "generate: " << ancfl::RSACipher::GenerateKey(pubkey_path,
    // prikey_path) << std::endl;
    ancfl::RSACipher::ptr rsa =
        ancfl::RSACipher::Create(pubkey_path, prikey_path);
    if (!rsa) {
