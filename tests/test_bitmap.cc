#include "ancfl/ds/bitmap.h"
#include "ancfl/ds/roaring_bitmap.h"
...
void init(uint32_t size, std::set<uint32_t> v = {}) {
    ...
    std::set<uint32_t> v0;
    ancfl::ds::Bitmap::ptr b;
    if(!size) {
        size = rand() % 4096;
        b.reset(new ancfl::ds::Bitmap(size));
    ...
    ancfl::ds::Bitmap::ptr c(new ancfl::ds::Bitmap(size, 0xFF));
    *c &= *b;
    ANCFL_ASSERT(c->getCount() == v0.size());
    ...
    //};
    //uint32_t size = 328;
    //ancfl::ds::Bitmap::ptr b(new ancfl::ds::Bitmap(size));
    //for(auto& i : vv) {
    //    b->set(i, true);
    //}
    ...
    ancfl::ds::Bitmap::ptr bb(new ancfl::ds::Bitmap(size, 0xFF));
    *b &= *bb;
#define XX(v) \
    ANCFL_LOG_INFO(g_logger) << #v ": " << ancfl::Join(v.begin(), v.end(), ",");
...
#undef XX
    ...
    //ANCFL_LOG_INFO(g_logger) << "size: " << size;
}
...
void test_compress(size_t size, std::set<uint32_t> v = {}) {
    ...
    if(*b != *bb) {
        ANCFL_LOG_INFO(g_logger) << "b  :" << b->toString();
        ANCFL_LOG_INFO(g_logger) << "bc :" << bc->toString();
        ANCFL_LOG_INFO(g_logger) << "bb :" << bb->toString();
        ANCFL_LOG_INFO(g_logger) << "size=" << size << " - " << ancfl::Join(v.begin(), v.end(), ",");
...
void test_op(size_t size, std::set<uint32_t> v1, std::set<uint32_t> v2) {
    ancfl::ds::Bitmap::ptr a;
    ancfl::ds::Bitmap::ptr b;

    if(size == 0) {
        size = rand() % 4096;
        a.reset(new ancfl::ds::Bitmap(size));
        b.reset(new ancfl::ds::Bitmap(size));
    ...
    ANCFL_ASSERT(a->cross(*b) == a->cross(*bc));
    ...
    ANCFL_ASSERT(a->cross(*b) == (!and_sv.empty()));
...
int main(int argc, char** argv) {
    srand(time(0));
    {
        ancfl::ds::Bitmap::ptr b(new ancfl::ds::Bitmap(504));
    }
...
std::vector<ancfl::ds::Bitmap::ptr> vs;
std::vector<ancfl::ds::Bitmap::ptr> vs2;
std::vector<ancfl::ds::RoaringBitmap::ptr> vs3;
...
void init() {
    ...
    vs3.resize(N);
    for (int i = 0; i < N; ++i) {
        ancfl::ds::Bitmap::ptr r(new ancfl::ds::Bitmap(M));
        ancfl::ds::RoaringBitmap::ptr r2(new ancfl::ds::RoaringBitmap());
        for (int i = 0; i < M / 1; ++i) {
            uint32_t v = rand() % M;
            r->set(v, true);
            r2->set(v, true);
...
void write_to_file(const std::string& name) {
...

    {
        ...
        a.reset(new ancfl::ds::RoaringBitmap);
        a->readFrom(ba);
...
void load_from_file(const std::string& name) {
    ...
    ba->setPosition(0);
    ancfl::ds::Bitmap::ptr a(new ancfl::ds::Bitmap(0));
    a->readFrom(ba);
    std::cout << "compress= " << a->isCompress()
...
void test_uncompress2() {
    for (int i = 0; i < N; ++i) {
        for (int n = i + 1; n < N; ++n) {
            ancfl::ds::Bitmap::ptr b(new ancfl::ds::Bitmap(M));
            *b |= *vs2[i];
        }
    }
}
    ...
    for (int i = 0; i < N; ++i) {
        for (int n = i + 1; n < N; ++n) {
            ...
            *b |= *vs2[i];
            ...
            ANCFL_ASSERT(*b == *vs2[i]->uncompress());
        }
    }
}
...
void test_uncompress4() {
    for (int i = 0; i < N; ++i) {
        for (int n = i + 1; n < N; ++n) {
            ancfl::ds::Bitmap::ptr b(new ancfl::ds::Bitmap(M, 0xff));
            *b &= *vs2[i];
...
void test_roaring_bitmap() {
    ancfl::ds::RoaringBitmap::ptr rb(new ancfl::ds::RoaringBitmap);
    for (int i = 0; i < 10; ++i) {
        rb->set(rand(), true);
    }
