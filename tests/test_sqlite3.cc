#include "ancfl/db/sqlite3.h"
#include "ancfl/log.h"
#include "ancfl/util.h"

static ancfl::Logger::ptr g_logger = ANCFL_LOG_ROOT();

void test_batch(ancfl::SQLite3::ptr db) {
    auto ts = ancfl::GetCurrentMS();
    int n = 1000000;
    ancfl::SQLite3Transaction trans(db);
    // db->execute("PRAGMA synchronous = OFF;");
    trans.begin();
    ancfl::SQLite3Stmt::ptr stmt = ancfl::SQLite3Stmt::Create(
...
int main(int argc, char** argv) {
    ...
    auto db = ancfl::SQLite3::Create(dbname, ancfl::SQLite3::READWRITE);
    if (!db) {
        ANCFL_LOG_INFO(g_logger) << "dbname=" << dbname << " not exists";
        db = ancfl::SQLite3::Create(
            dbname, ancfl::SQLite3::READWRITE | ancfl::SQLite3::CREATE);
        if (!db) {
            ANCFL_LOG_INFO(g_logger) << "dbname=" << dbname << " create error";
            return 0;
        }
...
#undef XX
        ...
        if (rt != SQLITE_OK) {
            ANCFL_LOG_ERROR(g_logger) << "create table error " << db->getErrno()
                                      << " - " << db->getErrStr();
            return 0;
        }
    }
    ...
    if (!stmt) {
        ANCFL_LOG_ERROR(g_logger) << "create statement error " << db->getErrno()
                                  << " - " << db->getErrStr();
        return 0;
    }
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // stmt->bind(3, ancfl::Time2Str(now + rand() % 100));
    ...
    ancfl::SQLite3Stmt::ptr query =
        ancfl::SQLite3Stmt::Create(db, "select * from user");
    if (!query) {
        ANCFL_LOG_ERROR(g_logger) << "create statement error " << db->getErrno()
                                  << " - " << db->getErrStr();
        return 0;
