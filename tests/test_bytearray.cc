...
#define XX(type, len, write_fun, read_fun, base_len)                  \
    {                                                                 \
        ...
        ancfl::ByteArray::ptr ba(new ancfl::ByteArray(base_len));     \
        for (auto& i : vec) {                                         \
            ba->write_fun(i);                                         \
        }                                                             \
        ba->setPosition(0);                                           \
        for (size_t i = 0; i < vec.size(); ++i) {                     \
            type v = ba->read_fun();                                  \
            ANCFL_ASSERT(v == vec[i]);                                \
        }                                                             \
        ANCFL_ASSERT(ba->getReadSize() == 0);                         \
...
#define XX(type, len, write_fun, read_fun, base_len)                         \
    {                                                                        \
        ...
        ancfl::ByteArray::ptr ba2(new ancfl::ByteArray(base_len * 2));       \
        ANCFL_ASSERT(                                                        \
            ba2->readFromFile("/tmp/" #type "_" #len "-" #read_fun ".dat")); \
        ba2->setPosition(0);                                                 \
        ANCFL_ASSERT(ba->toString() == ba2->toString());                     \
        ANCFL_ASSERT(ba->getPosition() == 0);                                \
        ANCFL_ASSERT(ba2->getPosition() == 0);                               \
    }
    XX(int8_t, 100, writeFint8, readFint8, 1);
    XX(uint8_t, 100, writeFuint8, readFuint8, 1);
    XX(int16_t, 100, writeFint16, readFint16, 1);
